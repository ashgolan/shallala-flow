              ...
              <div className="form-group">
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button type="button" onClick={() => setPaidFully(true)}
                    style={{
                      flex: 1, padding: '8px 0', borderRadius: 8, fontWeight: 700, cursor: 'pointer',
...
export default function AdminReadings({ adminRole = 'admin' }) {
  ...
  const projectsTotalsByFarmer = (() => {
    ...
    projects.forEach(p => {
      (p.members || []).forEach(m => {
        ...
        if (m.amount === null || m.amount === undefined) return;
        const paid = (m.payments || []).reduce((s, pay) => s + (parseFloat(pay.amount) || 0), 0);
        if (!map[m.farmerId]) map[m.farmerId] = { required: 0, paid: 0 };
        map[m.farmerId].required += m.amount;
        map[m.farmerId].paid += paid;
      });
    });
    return map;
  })();

  // ✅ فقط للمزارعين الظاهرين حالياً بالجدول (حسب الفلاتر)، بدون تكرار
  const projectsTotals = (() => {
    const farmerIdsShown = new Set(filtered.map(r => r.farmerId));
    let required = 0, paid = 0;
    farmerIdsShown.forEach(fid => {
      const tt = projectsTotalsByFarmer[fid];
      if (tt) { required += tt.required; paid += tt.paid; }
    });
    return { required, paid };
  })();

  // ✅ الإجمالي الشامل: مياه + إضافات + مشاريع — المدفوع مقابل الكامل
  const combinedTotal = paymentProgress.total + extrasGrossTotal + projectsTotals.required;
  const combinedPaid = paymentProgress.paid + extrasPaidTotal + projectsTotals.paid;
  ...
  return (
    <div>
      ...
      {/* ✅ سطر جديد: الإجمالي الشامل (مياه + إضافات + مشاريع) */}
      <div style={{ textAlign: 'center', marginBottom: 16 }}>
        <span style={{ fontSize: 13, fontWeight: 800, color: 'var(--text-secondary)' }}>
          💰 {ar ? 'الإجمالي الشامل (مياه + إضافات + مشاريع)' : 'סה"כ כולל (מים + תוספות + פרויקטים)'}:
        </span>{' '}
        <span style={{ fontSize: 14, fontWeight: 900, color: combinedPct >= 100 ? '#16a34a' : '#dc2626' }}>
