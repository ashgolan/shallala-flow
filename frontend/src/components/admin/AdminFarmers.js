...
export default function AdminFarmers({ adminRole = 'admin' }) {
  ...
  // ✅ مشاريع غير مدفوعة لكل مزارع — Map: farmerId → [{ projectName, remaining }]
  const unpaidProjectsByFarmer = (() => {
    const map = {};
    projects.forEach(p => {
      (p.members || []).forEach(m => {
        if (!m.farmerId) return; // تجاهل الأسماء الحرة (customMembers) غير المربوطة بمزارع
        ...
        if (remaining > 0.01) {
          const key = String(m.farmerId);
          if (!map[key]) map[key] = [];
          map[key].push({ projectName: p.name, remaining });
        }
      });
    });
    return map;
  ...
  const projectsUnpaidTotal = farmerId =>
    (unpaidProjectsByFarmer[String(farmerId)] || []).reduce((s, p) => s + p.remaining, 0);
  const calcUnpaid = farmerId => {
    const readingsTotal = readings
  ...
  const ROWS_PER_PAGE_PDF = 20;

  const buildDebtorsPageHTML = (rows, pageIndex, totalPages, grandTotal, totalCount, dateStr) => {
    const GREEN = '#166534', GREEN_DARK = '#14532d', RED = '#dc2626', RED_LIGHT = '#fff1f2';
    const title = ar ? 'كشف المزارعين أصحاب الديون' : 'דו"ח חקלאים עם חוב';

    const rowsHtml = rows.map((d, i) => {
      const idx = pageIndex * ROWS_PER_PAGE_PDF + i + 1;
      const bg = i % 2 === 0 ? '#ffffff' : '#f9fafb';
      const name = `${d.farmer.lastName || ''} ${d.farmer.firstName || ''}`.trim() || d.farmer.nameHeb || d.farmer.name || '—';

      // ✅ سطر صغير تحت الاسم — تفصيل المشاريع المستحقة (إن وجدت)
      const projectsNote = (d.projects && d.projects.length)
        ? `<div style="margin-top:3px;font-size:10px;color:#7c3aed;font-weight:700;line-height:1.5;">
         ${d.projects.map(p => `🏗️ ${p.projectName}: ₪${Math.round(p.remaining).toLocaleString()}`).join(' &nbsp;•&nbsp; ')}
       </div>`
        : '';

      return `
    <tr style="background:${bg};">
      <td style="padding:9px 10px;text-align:center;border-bottom:1px solid #e5e7eb;font-weight:700;color:#94a3b8;font-size:12px;vertical-align:top;">${idx}</td>
      <td style="padding:9px 10px;text-align:right;border-bottom:1px solid #e5e7eb;vertical-align:top;">
        <div style="font-weight:800;font-size:14px;">${name}</div>
        ${projectsNote}
      </td>
      <td style="padding:9px 10px;text-align:center;border-bottom:1px solid #e5e7eb;color:#555;font-size:13px;vertical-align:top;">${d.farmer.phone || '—'}</td>
      <td style="padding:9px 10px;text-align:center;border-bottom:1px solid #e5e7eb;vertical-align:top;">
        <span style="display:inline-block;background:#fff1f2;color:#dc2626;border:1px solid #fecaca;border-radius:8px;padding:4px 14px;font-weight:900;font-size:14px;">
    ...
    return `
  <div dir="rtl" style="width:800px;padding:32px;font-family:'Heebo',Tahoma,Arial,sans-serif;background:#fff;color:#1a1a1a;">
    <div style="background:linear-gradient(135deg,${GREEN},${GREEN_DARK});border-radius:16px;padding:20px 26px;margin-bottom:18px;display:flex;justify-content:space-between;align-items:center;">
      <div>
        <div style="color:#fff;font-size:22px;font-weight:900;">🌿 ${ar ? 'الشلالة' : 'השלאלה'}</div>
        <div style="color:#bbf7d0;font-size:13px;margin-top:3px;">${title} — ${totalCount} ${ar ? 'مزارع' : 'חקלאים'}</div>
      </div>
      <div style="text-align:left;color:#fff;font-size:12px;line-height:1.6;">
        <div>${ar ? 'التاريخ' : 'תאריך'}: ${dateStr}</div>
        ${totalPages > 1 ? `<div>${ar ? 'صفحة' : 'עמוד'} ${pageIndex + 1} / ${totalPages}</div>` : ''}
      </div>
    </div>

    <table style="width:100%;border-collapse:collapse;border-radius:12px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,0.1);">
      <thead>
        <tr style="background:${GREEN};">
          <th style="padding:10px;color:#fff;font-size:12px;width:36px;">#</th>
          <th style="padding:10px;color:#fff;font-size:12px;text-align:right;">${ar ? 'اسم المزارع' : 'שם החקלאי'}</th>
          <th style="padding:10px;color:#fff;font-size:12px;">${ar ? 'الهاتف' : 'טלפון'}</th>
          <th style="padding:10px;color:#fff;font-size:12px;">${ar ? 'المبلغ غير المدفوع' : 'יתרה לתשלום'}</th>
        </tr>
      </thead>
    ...
    <div style="margin-top:20px;background:${RED};border-radius:14px;padding:16px 26px;display:flex;justify-content:space-between;align-items:center;">
      <span style="color:#fee2e2;font-weight:800;font-size:14px;">💰 ${ar ? 'الإجمالي المستحق على الجميع' : 'סה"כ לתשלום'}</span>
      <span style="color:#fff;font-weight:900;font-size:26px;">₪${Math.round(grandTotal).toLocaleString()}</span>
    </div>` : ''}
  </div>`;
  };

  const exportUnpaidFarmersPDF = async () => {
    setPdfLoading(true);
    try {
      const { jsPDF } = await import('jspdf');
      const html2canvas = (await import('html2canvas')).default;

      // ✅ نفس منطق حساب الرصيد غير المدفوع بالضبط المستخدم بعمود "غير مدفوع" (فترة فترة)
      const debtors = farmers
        .map(f => ({
          farmer: f,
          amount: calcUnpaid(f.id),
          projects: unpaidProjectsByFarmer[String(f.id)] || [], // ✅ جديد
        }))
        .filter(x => x.amount > 0.01)
        .sort((a, b) => b.amount - a.amount);
      if (!debtors.length) {
        alert(ar ? 'لا يوجد مزارعون عليهم ديون حالياً 🎉' : 'אין חקלאים עם חוב כרגע 🎉');
        setPdfLoading(false); return;
      }

      const grandTotal = debtors.reduce((s, d) => s + d.amount, 0);
      const pages = [];
      for (let i = 0; i < debtors.length; i += ROWS_PER_PAGE_PDF) pages.push(debtors.slice(i, i + ROWS_PER_PAGE_PDF));
      const dateStr = new Date().toLocaleDateString(ar ? 'ar-EG-u-nu-latn' : 'he-IL');
      ...
      for (let p = 0; p < pages.length; p++) {
        container.innerHTML = buildDebtorsPageHTML(pages[p], p, pages.length, grandTotal, debtors.length, dateStr);
      ...
      pdf.save(`alshallala-debtors-${new Date().toISOString().slice(0, 10)}.pdf`);
    } catch (e) {
      console.error(e);
      alert(ar ? 'خطأ في إنشاء PDF: ' + e.message : 'שגיאה ביצירת PDF: ' + e.message);
    }
    setPdfLoading(false);
  };
  // ════════════════════════════════════════
      ...
      <div>
        ...
        <div className="flex-between mb-20" style={{ flexWrap: 'wrap', gap: 12 }}>
          ...
          <div className="flex-gap gap-8">
            ...
            <button className="btn btn-outline" onClick={exportUnpaidFarmersPDF} disabled={pdfLoading}
              style={{ background: '#fef2f2', border: '1.5px solid #dc2626', color: '#b91c1c', fontWeight: 700 }}
              title={ar ? 'تحميل PDF بالمزارعين أصحاب الديون' : 'הורד PDF של חקלאים עם חוב'}>
              {pdfLoading ? '⏳...' : '📄 PDF'}
            </button>
            {!isViewer && (
              <>
                                            ...
                                            {(() => {
                                              ...
                                              return Object.entries(grouped).sort(([a], [b]) => a.localeCompare(b)).map(([code, lands]) => (
                                                <optgroup key={code} label={`── ${code} ──`}>
                                                  {lands.map(l => {
                                                    const reg = regions.find(r => r.id === l.regionId); return (
