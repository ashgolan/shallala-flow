...
export default function FarmerDashboard({ farmer: farmerProp, onLogout }) {
  ...
  const farmer = farmerProp || (() => {
    try { return stored && stored !== 'undefined' ? JSON.parse(stored) : null; } catch { return null; }
  })(); const { lang } = useLang();
                ...
                <div key={i} className={`stat-card ${s.accent ? 'accent' : ''}`}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <div style={{
                        fontWeight: 900, fontSize: 'clamp(1rem,5vw,1.6rem)', lineHeight: 1.1,
                      ...
                      <div style={{
                        fontSize: 'clamp(10px,2.5vw,12px)', marginTop: 3, opacity: 0.75,
      ...
      <nav style={{ display: 'none', position: 'fixed', bottom: 0, left: 0, right: 0, background: '#fff', borderTop: '1.5px solid var(--border)', zIndex: 200, boxShadow: '0 -4px 12px rgba(0,0,0,0.08)' }} className="mobile-bottom-nav">
        {tabs.map(tb => (
          <button key={tb.key} onClick={() => setTab(tb.key)}
            style={{
              flex: 1, padding: '10px 4px', border: 'none', background: 'transparent', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3,
